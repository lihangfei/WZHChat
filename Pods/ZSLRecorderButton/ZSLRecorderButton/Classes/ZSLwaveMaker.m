...
#import "ZSLwaveLayer.h"

#ifdef DEBUG
    #define ZSLwarningLog(format, ...)  NSLog(@"[ZSLwaveWarning]: " format, ##__VA_ARGS__);
    #define BMLog(format, ...)  NSLog(@"[ZSLwaveLog]: " format, ##__VA_ARGS__);
#else
    #define ZSLwarningLog(format, ...)
    #define BMLog(format, ...) 
#endif

#define chargeViewIsValid if (!self.animationView) {\
                                ZSLwarningLog(@"%@",kBMViewError);\
                                return;\
                            }
...
@interface ZSLwaveMaker () <ZSLwaveLayerDelegate>
{
...
- (void)span {
    ...
    ZSLwaveLayer *waveLayer = [ZSLwaveLayer layer];
